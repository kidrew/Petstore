...
async function mintToken(pet) {
    ...
    // The metadata contains the attributes `url` which is an IFPS URL pointing to the data.json.
    const { url } = await uploadToStorage(pet);

    // We want to include the IPFS URL to the blockchain, so we can "unpack" the token data when we query it later.
    // So we create a new object with all the pet's attributes plus `url`.
...
async function uploadToStorage(pet) {
    // Call 'store(...)' on the NFTStorage client with an object containing all of pet's attributes, required image and description attributes.
