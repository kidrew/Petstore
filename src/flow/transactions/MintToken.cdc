...
transaction(metadata: {String: String}) {
    ...
    // 'prepare' block always take one or more 'AuthAccount' parameter(s) to indicate who are signing the transaction.
    // It takes the account info of the user trying to execute the transaction and validate. In this case, the contract owner's account.
    // Here we try to "borrow" the capabilities available on 'NFTMinter' and 'NFTReceiver' resources, and will fail if the user executing this transaction does not have access to these resources.
    prepare(account: AuthAccount) {

        // Note that we have to call 'getCapability(_ domain: Domain)' on the account object before we can 'borrow()'.
        ...
        // Note that 'NFTMinter' is borrowed from '/storage' domain namespace, which means it is only accessible to this account.
